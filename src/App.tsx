...
function App() {
  ...
  // View Mode: Added 'reviews'
  const [viewMode, setViewMode] = useState<'inspector' | 'raw' | 'json' | 'reference' | 'settings' | 'contact' | 'reviews'>('inspector');
  ...
  return (
    <div className="flex flex-col h-screen bg-white dark:bg-slate-950 text-gray-900 dark:text-slate-200 font-sans overflow-hidden transition-colors duration-200 animate-fade-in">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {(viewMode !== 'reference' && viewMode !== 'settings' && viewMode !== 'contact' && viewMode !== 'reviews' && (doc || rawEdi)) && (
            <div 
      ...
      {doc && viewMode !== 'reference' && viewMode !== 'settings' && viewMode !== 'contact' && viewMode !== 'reviews' && <ChatInterface rawEdi={rawEdi} />}
    </div>
  );
}
